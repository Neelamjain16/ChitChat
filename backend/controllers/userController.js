...
const getSuggestedUsers=async(req,res)=>{
  try {
    //exclude the current user from suggested user array,exclude the users that current user is already following
    const userId=req.user._id;
    ...
    res.status(500).json({error:err.message});
  console.log("Error in getsuggestedUser:",err)
  }
}
...
const freezeAccount=async(req,res)=>{
  try {
    ...
    res.status(500).json({error:err.message});
    console.log("Error in freezeAccount:",err)
  }
}
